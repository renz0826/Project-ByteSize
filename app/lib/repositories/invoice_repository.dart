...
class InvoiceRepository {
  ...
  Future<List<JoinedInvoice>> getAllInvoices() async {
    try {
      ...
      for (final inv in invoices) {
        ...
        final patientName = patient != null ? '${patient.firstName} ${patient.lastName}' : 'Unknown Patient';
  ...
  Future<int> createInvoice({
    ...
    return _db.transaction(() async {
      final patient = await (_db.select(_db.patient)..where((p) => p.patientId.equals(patientId))).getSingle();
  ...
  Future<void> processPayment({
    ...
    return _db.transaction(() async {
      ...
      final patient = await (_db.select(_db.patient)..where((t) => t.patientId.equals(invoice.patientId))).getSingle();
      ...
      final charges = await (_db.select(_db.procedureCharge)..where((t) => t.invoiceId.equals(invoiceId))).get();
      final grandTotal = charges.fold(0.0, (sum, c) => sum + c.totalProcedureCharge);
      final hasDiscount = _isDiscountApplicable(patient);
      ...
      final totalPaidSoFar = payments.fold(0.0, (sum, p) => sum + p.amountReceived);

      // 4. Calculate exact absolute balance (Fixes any old buggy data instantly)
      final newRemainingBalance = (netTotal - totalPaidSoFar).clamp(0.0, double.infinity);
